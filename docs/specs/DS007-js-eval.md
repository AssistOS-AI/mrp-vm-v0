...
When a `js-eval` step is meant to populate its declared target family directly without calling `sop.emit()` or `~family.set()`, the preferred planner-facing form is an explicit final `return ...;` statement. The runtime may treat one simple final expression as shorthand for that return, but KU guidance should still steer planning toward the explicit form so emitted outputs remain obvious in the source.
