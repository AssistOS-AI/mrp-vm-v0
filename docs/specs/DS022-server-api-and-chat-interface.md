...
1. **Session management**: create a new session, select an existing session from a compact header selector, reuse the current active session when the user returns to `/chat`, and surface the active session identity, origin, effective role, last activity time, and status in the header rather than in a bulky first-page panel. Page load must not silently create a replacement session when an existing one is already selected or when no session is needed yet.
...
5. **Advanced options**: support text-file insertion into the input area, budget controls, and a small menu of predefined demo tasks that exercise distinct commands and interpreters. The demo-task catalog should come from repository-owned shared fixtures rather than duplicated browser-only constants, should stay compact enough to scan quickly, and should favor interpreter-neutral task wording plus explicit execution/output requirements over prompts that hard-code a named interpreter.
