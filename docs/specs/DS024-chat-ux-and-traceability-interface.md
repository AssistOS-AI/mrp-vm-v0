...
7. explicit dependency arrows between layers that update when layer columns are resized,
8. resizable layer columns whose `Layer N` heading acts as the horizontal drag handle so dragging a heading changes that column width and shifts later columns without exposing separate resize bars, while node cards stay fixed-width and centered in the column,
9. visible vertical dashed guides per layer, aligned with the layer heading so the column structure remains readable even when arrows are thin,
10. execution-graph nodes that open inspection details on click but are not themselves draggable,
11. a default width strategy that distributes available viewport width across layers by changing column-side padding around fixed-width node cards, so the graph fills horizontal space without unnecessary scrolling when the layers fit,
